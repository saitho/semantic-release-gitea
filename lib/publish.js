const {basename, resolve, parse} = require('path');
...
module.exports = async (pluginConfig, context) => {
  ...
  await Promise.all(
    globbedAssets.map(async asset => {
      ...
      // Replace all slashes with underscores, as Gitea (or anything else, probably) does not allow slashes in asset names
      // Also parsing might have a problem with slashes in the name
      const assetPath = parse(assetName.replace(/\//g, '_'));
      if (isPlainObject(asset) && asset.label) {
          assetName = template(asset.label)({...context, stem: assetPath.name, ext: assetPath.ext});
      }
