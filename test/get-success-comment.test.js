...
const HOME_URL = 'https://gitea.io/semantic-release/semantic-release';

test('Comment for issue with multiple releases', t => {
  const issue = {number: 1};
  const releaseInfos = [
    {name: 'GitHub release', url: 'https://gitea.io/release'},
...
- [GitHub release](https://gitea.io/release)
...
test('Comment with missing release URL', t => {
  const issue = {number: 1};
  const releaseInfos = [{name: 'GitHub release', url: 'https://gitea.io/release'}, {name: 'npm release'}];
  const nextRelease = {version: '1.0.0'};
  const comment = getSuccessComment(issue, releaseInfos, nextRelease);
...
test('Comment with one release', t => {
  const issue = {number: 1};
  const releaseInfos = [{name: 'GitHub release', url: 'https://gitea.io/release'}];
  const nextRelease = {version: '1.0.0'};
  const comment = getSuccessComment(issue, releaseInfos, nextRelease);
...
The release is available on [GitHub release](https://gitea.io/release)
