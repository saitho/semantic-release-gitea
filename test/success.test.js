...
test.serial(
  ...
  async t => {
    ...
    const commits = [
      ...
      {
        hash: '789',
        message: `Commit 3 message Closes https://gitea.io/${redirectedOwner}/${redirectedRepo}/issues/4`,
        tree: {long: 'ccc'},
      },
    ];
    ...
    const github = authenticate(env)
      ...
      .reply(200, {html_url: 'https://gitea.io/successcomment-2'})
      ...
      .reply(200, {html_url: 'https://gitea.io/successcomment-3'})
      ...
      .reply(200, {html_url: 'https://gitea.io/successcomment-4'})
    ...
    t.true(t.context.log.calledWith('Added comment to issue #%d: %s', 2, 'https://gitea.io/successcomment-2'));
    t.true(t.context.log.calledWith('Added labels %O to issue #%d', ['released'], 2));
    t.true(t.context.log.calledWith('Added comment to issue #%d: %s', 3, 'https://gitea.io/successcomment-3'));
    t.true(t.context.log.calledWith('Added labels %O to issue #%d', ['released'], 3));
    t.true(t.context.log.calledWith('Added comment to issue #%d: %s', 4, 'https://gitea.io/successcomment-4'));
...
test.serial(
  ...
  async t => {
    const owner = 'test_user';
    const repo = 'test_repo';
    const env = {GITEA_URL: 'https://custom-url.com', GITEA_TOKEN: 'gitea_token', GITEA_PREFIX: 'prefix'};
    const failTitle = 'The automated release is failing 🚨';
    const pluginConfig = {failTitle};
    const prs = [
      {number: 1, pull_request: {}, state: 'closed'},
...
test.serial('Make multiple search queries if necessary', async t => {
  ...
  t.true(t.context.log.calledWith('Added comment to issue #%d: %s', 4, 'https://gitea.io/successcomment-4'));
  t.true(t.context.log.calledWith('Added labels %O to issue #%d', ['released'], 4));
  t.true(t.context.log.calledWith('Added comment to issue #%d: %s', 5, 'https://gitea.io/successcomment-5'));
  t.true(t.context.log.calledWith('Added labels %O to issue #%d', ['released'], 5));
  t.true(t.context.log.calledWith('Added comment to issue #%d: %s', 6, 'https://gitea.io/successcomment-6'));
...
test.serial('Ignore errors when adding comments and closing issues', async t => {
  ...
  const github = authenticate(env)
    ...
    .reply(200, {html_url: 'https://gitea.io/issues/3'});
  ...
  t.true(t.context.log.calledWith('Added comment to issue #%d: %s', 2, 'https://gitea.io/successcomment-2'));
  t.true(t.context.log.calledWith('Closed issue #%d: %s.', 3, 'https://gitea.io/issues/3'));
  t.true(github.isDone());
});
...
test.serial('Close open issues when a release is successful', async t => {
  ...
  const github = authenticate(env)
    ...
    .reply(200, {items: issues})
    .patch(`/repos/${owner}/${repo}/issues/2`, {state: 'closed'})
    .reply(200, {html_url: 'https://gitea.io/issues/2'})
    .patch(`/repos/${owner}/${repo}/issues/3`, {state: 'closed'})
    .reply(200, {html_url: 'https://gitea.io/issues/3'});
  ...
  t.true(t.context.log.calledWith('Closed issue #%d: %s.', 2, 'https://gitea.io/issues/2'));
  t.true(t.context.log.calledWith('Closed issue #%d: %s.', 3, 'https://gitea.io/issues/3'));
  t.true(github.isDone());
});
