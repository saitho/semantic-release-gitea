...
test.serial('Publish a release with one asset and custom github url', async t => {
  ...
  const nextRelease = {gitTag: 'v1.0.0', name: 'v1.0.0', notes: 'Test release note body'};
  const options = {repositoryUrl: `https://gitea.io/${owner}/${repo}.git`};
  const untaggedReleaseUrl = `${env.GITEA_URL}/${owner}/${repo}/releases/untagged-123`;
  const releaseUrl = `${env.GITEA_URL}/${owner}/${repo}/releases/${nextRelease.version}`;
  const assetUrl = `${env.GITEA_URL}/${owner}/${repo}/releases/download/${nextRelease.version}/upload.txt`;
  const releaseId = 1;
  const uploadUri = `/api/uploads/repos/${owner}/${repo}/releases/${releaseId}/assets`;
  const uploadUrl = `${env.GITEA_URL}${uploadUri}{?name,label}`;
